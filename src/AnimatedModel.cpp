...
namespace pacsphere {
    ...
    thread_local Assimp::Importer AnimatedModel::_importer;
    std::mutex AnimatedModel::_mutex;
    
    AnimatedModel::AnimatedModel(const std::string &filename, const double scale, glm::vec4 materialColor /*=glm::vec4(1.0)*/) : _materialColor(materialColor)
    {
    ...
    AnimatedModel::AnimatedModel(const std::string &fileContents, glm::vec4 materialColor /*=glm::vec4(1.0)*/) : _materialColor(materialColor)
    {
        //Assimp::Logger::LogSeverity severity = Assimp::Logger::NORMAL;
        // Create a logger instance for Console Output
        //Assimp::DefaultLogger::create("", severity, aiDefaultLogStream_STDOUT);
    ...
    void AnimatedModel::importMeshFromString(const std::string &fileContents) {
        /*
         if (_importer.get() == nullptr) {
         _importer.reset(new Assimp::Importer());
         }
    ...
    std::shared_ptr<AnimatedMesh> AnimatedModel::processMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4 scaleMat)
    {
        // Data to fill
        std::vector<AnimatedMesh::Vertex> cpuVertexArray;
        ...
        std::shared_ptr<AnimatedMesh> gpuMesh(new AnimatedMesh(textures, GL_TRIANGLES, GL_STATIC_DRAW, cpuVertexByteSize, cpuIndexByteSize, 0, cpuVertexArray, cpuIndexArray.size(), cpuIndexByteSize, &cpuIndexArray[0]));
    ...
    std::vector<std::shared_ptr<Texture> > AnimatedModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type)
    {
        ...
        for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // If texture hasn't been loaded already, load it
                ...
                this->_textures.push_back(texture);  // Store it as texture loaded for entire AnimatedModel, to ensure we won't unnecesery load duplicate textures.
            }
        }
        return textures;
    }
    ...
    void AnimatedModel::setMaterialColors(const std::vector<glm::vec4> &colors) {
        ...
        if (numMeshes != numColors) {
            
            cout << "Number of colors and meshes don't match." << endl << endl << "Meshes: " << numMeshes << endl << endl << "Colors: " << numColors;
            cout << endl << "All meshes will be set to white" << endl << endl;
            
            this->setMaterialColor(vec4(1, 1, 1, 1));
        }
        else {
            
            for (int i = 0; i < numColors; i++) {
                
                _meshes[i]->setMaterialColor(colors[i]);
            }
        }
    }
}
